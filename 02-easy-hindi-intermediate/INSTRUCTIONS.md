...
4. **A tiny pure-Hindi story or article: 4–8 authored lines.** Each line is one short sentence, not a long paragraph that happens to wrap. First display the complete passage pronunciation in Bangla script as 4–8 lines; do not print a native-only passage. Next display the same lines as a native word-by-word breakdown, with Bangla pronunciation immediately after every target word. Then show the **complete article/story pronunciation in Bangla**, and finally its **complete Bangla meaning**. This display order applies to desktop, mobile, samples and both EPUBs. Keep all native source strings and pronunciation metadata; native script is displayed only with immediate word-level Bangla pronunciation. The mixed and pure readings share the topic but use different situations or viewpoints. Use simple, familiar, single-clause sentences, at most eight word/learning units per line. Foundation chapters with a smaller sentence ceiling keep that smaller limit. Reuse taught vocabulary and avoid long linked clauses. This rule remains simple even in Intermediate and later vocabulary chapters.
...
Keep each native word beside its pronunciation, including after mobile reflow. Follow the six-section order above. The pure reading appears first as 4–8 complete pronunciation lines written in Bangla, then the native word-by-word breakdown with immediate Bangla pronunciation, then full Bangla pronunciation and Bangla meaning. Use the mixed reading’s checked `text_md` representation with every native word wrapped in Markdown `**bold**`; do not highlight Bangla narration as though it were target text. For Arabic, isolate RTL native words and LTR Bangla cues; never reverse strings. Native combining marks and word units must remain intact. Publisher text is `Published by Englishing.app`, typeset by the layout.
