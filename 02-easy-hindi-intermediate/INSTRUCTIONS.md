# Easy Hindi Intermediate — authoring instructions
...
Intermediate develops the communicative goals in its chapter plans while keeping full script and pronunciation help. Its pure-language readings remain simple, even when its sentence lessons introduce more intermediate communication.

## 2. The same chapter order every time

1. **Exactly 20 essential sentences.** Display natural Bangla meaning first, then the complete sentence pronounced in Bangla script, then its native word-by-word breakdown with Bangla pronunciation immediately after every word. Keep sentences useful and natural. Retain romanization as an additional aid; it never replaces Bangla pronunciation.
...
4. **A tiny pure-Hindi story or article: 4–8 authored lines.** Each line is one short sentence, not a long paragraph that happens to wrap. Display the existing full Bangla-script pronunciation first, then the complete Bangla meaning, and finally the native word-by-word breakdown with adjacent Bangla pronunciation. Use the existing text without rewriting or adding another full-passage copy. Do not add pronunciation headings or repeated “উচ্চারণ:” labels in this reading section. Preserve the existing visual styling. Keep all native source strings and pronunciation metadata; native script is displayed only with immediate word-level Bangla pronunciation. The mixed and pure readings share the topic but use different situations or viewpoints. Use simple, familiar, single-clause sentences, at most eight word/learning units per line. Foundation chapters with a smaller sentence ceiling keep that smaller limit. Reuse taught vocabulary and avoid long linked clauses. This rule remains simple even in Intermediate and later vocabulary chapters.
...
**Beginner display rule for every edition:** Readers cannot read the target script. Sentence and vocabulary entries show Bangla meaning first → the full target-language sentence/word pronounced in Bangla script → the target-language word-by-word breakdown, with Bangla pronunciation immediately after every native word. Romanization is an additional aid. Never show a standalone native-only sentence, word or passage. Target script appears only in annotated breakdowns, mixed-reading annotated spans and script/number teaching with adjacent Bangla pronunciation. Desktop uses A4 portrait tables; mobile and both EPUBs use stacked entries in the same order. Keep native strings in source JSON for validation; do not print them as unsupported text.
...
Keep each native word beside its pronunciation, including after mobile reflow. Follow the six-section order above. The pure reading displays existing full Bangla-script pronunciation, then Bangla meaning, then annotated native breakdown. Do not add pronunciation labels or duplicate full passages. Use the mixed reading’s checked `text_md` representation with every native word wrapped in Markdown `**bold**`; do not highlight Bangla narration as though it were target text. For Arabic, isolate RTL native words and LTR Bangla cues; never reverse strings. Native combining marks and word units must remain intact. Publisher text is `Published by Englishing.app`, typeset by the layout.
...
python3 scripts/manage.py validate --book 02-easy-hindi-intermediate --complete
python3 scripts/manage.py assemble --book 02-easy-hindi-intermediate
```
...
A verified PR containing the requested chapter files is the required delivery. If PR creation fails temporarily (for example a transient service error or rate limit), wait **2–3 minutes between attempts**, or longer if GitHub requests it, then retry. Continue retrying temporary failures until the PR is created and verified, for as long as the active session can continue. Do not stop after the first failure or treat a pushed branch alone as finished delivery. Use real waiting tools; split the wait into shorter intervals when needed to keep the session responsive. Never claim to wait or retry after the session ends.

Before every retry, check for an existing PR for the same branch/book/range. A timed-out creation request may already have succeeded: reuse and verify that PR instead of creating a duplicate. Preserve completed files and commits, record actual attempt times and error messages in `authoring-progress.json`, and retry the failed delivery step without regenerating completed chapters. Verify the PR URL, base `main`, and requested source-file diff before reporting success.

Retrying does not grant missing permissions. If authentication, repository access, an explicit security/policy restriction or a hard session limit requires outside action, preserve the finished files and branch, explain the exact blocker and attempts made, and mark delivery incomplete. Do not bypass restrictions or promise unattended continuation. If writing tools are unavailable, files may be returned as a recovery copy, but that is not completed PR delivery. If all requested chapters already exist correctly on `main`, retain the existing no-empty-PR exception. Whole-book completion claims require the full-book checks above; a batch PR does not require full-book assembly. The manager handles artwork, typography, publication and parent submodule updates after merging.
