#!/usr/bin/env python3
"""Build review PDFs and EPUBs from validated chapter JSON and artwork for Easy Hindi Intermediate."""
...
def blend(c1: tuple[int, int, int], c2: tuple[int, int, int], alpha: float) -> tuple[int, int, int]:
    return tuple(int(c1[i] * (1.0 - alpha) + c2[i] * alpha) for i in range(3))


def synthesize_cover(path: Path, ink_hex: str = "#175B72", bg_hex: str = "#EAF5FA") -> None:
    """Clean editorial cover placeholder (1024x1536 RGB) with calm upper third for typography."""
    w, h = 1024, 1536
    scale = 2
    ...
    margin = 40 * scale
    draw.rectangle([margin, margin, sw - margin, sh - margin], outline=blend(bg, ink, 0.28), width=3 * scale)
    draw.rectangle([margin + 12 * scale, margin + 12 * scale, sw - margin - 12 * scale, sh - margin - 12 * scale],
                   outline=blend(bg, ink, 0.14), width=1 * scale)

    # Upper third is kept calm for typography. Lower two-thirds has geometric editorial artwork.
    cx = sw // 2
    base_y = int(1050 * scale)

    draw.ellipse([cx - 360 * scale, base_y - 360 * scale, cx + 360 * scale, base_y + 360 * scale], fill=blend(bg, ink, 0.08))
    draw.ellipse([cx - 260 * scale, base_y - 200 * scale, cx + 260 * scale, base_y + 320 * scale], fill=blend(bg, ink, 0.14))

    arch_w = 200 * scale
    arch_top = base_y - 250 * scale
    draw.rounded_rectangle([cx - arch_w, arch_top, cx + arch_w, base_y + 300 * scale], radius=arch_w, fill=blend(bg, ink, 0.22))
    draw.rounded_rectangle([cx - arch_w + 30 * scale, arch_top + 30 * scale, cx + arch_w - 30 * scale, base_y + 270 * scale],
                           radius=arch_w - 30 * scale, fill=bg)

    draw.ellipse([cx - 60 * scale, arch_top + 80 * scale, cx + 60 * scale, arch_top + 200 * scale], fill=blend(bg, ink, 0.35))
    draw.ellipse([cx - 30 * scale, arch_top + 110 * scale, cx + 30 * scale, arch_top + 170 * scale], fill=paper)

    draw.line([(cx - 320 * scale, base_y + 320 * scale), (cx + 320 * scale, base_y + 320 * scale)], fill=blend(bg, ink, 0.3), width=2 * scale)
...
def synthesize_banner(path: Path, ink_hex: str = "#175B72", bg_hex: str = "#EAF5FA") -> None:
    """Clean editorial banner placeholder (1536x1024 RGB) with calm left third for typography."""
    w, h = 1536, 1024
    scale = 2
    sw, sh = w * scale, h * scale
    ink = hex_to_rgb(ink_hex)
    ...
    margin = 36 * scale
    draw.rectangle([margin, margin, sw - margin, sh - margin], outline=blend(bg, ink, 0.22), width=3 * scale)

    cx = int(1050 * scale)
    cy = sh // 2

    draw.ellipse([cx - 320 * scale, cy - 320 * scale, cx + 320 * scale, cy + 320 * scale], fill=blend(bg, ink, 0.08))
    draw.ellipse([cx - 200 * scale, cy - 200 * scale, cx + 200 * scale, cy + 200 * scale], fill=blend(bg, ink, 0.16))

    arch_w = 160 * scale
    draw.rounded_rectangle([cx - arch_w, cy - 180 * scale, cx + arch_w, cy + 220 * scale], radius=arch_w, fill=blend(bg, ink, 0.24))
    draw.rounded_rectangle([cx - arch_w + 25 * scale, cy - 155 * scale, cx + arch_w - 25 * scale, cy + 195 * scale],
                           radius=arch_w - 25 * scale, fill=bg)

    draw.ellipse([cx - 45 * scale, cy - 80 * scale, cx + 45 * scale, cy + 10 * scale], fill=blend(bg, ink, 0.35))
    draw.ellipse([cx - 20 * scale, cy - 55 * scale, cx + 20 * scale, cy - 15 * scale], fill=paper)

    card_color = blend(bg, ink, 0.12)
    draw.rounded_rectangle([cx + 180 * scale, cy - 120 * scale, cx + 340 * scale, cy + 160 * scale], radius=20 * scale, fill=card_color)
    draw.rounded_rectangle([cx - 340 * scale, cy - 60 * scale, cx - 180 * scale, cy + 180 * scale], radius=20 * scale, fill=card_color)
...
def synthesize_reading(path: Path, is_bridge: bool, ch_num: int, ink_hex: str, bg_hex: str) -> None:
    """Clean editorial chapter reading placeholder (1536x1024 RGB)."""
    w, h = 1536, 1024
    scale = 2
    sw, sh = w * scale, h * scale
    ink = hex_to_rgb(ink_hex)
    ...
    margin = 40 * scale
    draw.rectangle([margin, margin, sw - margin, sh - margin], outline=blend(bg, ink, 0.25), width=3 * scale)
    draw.rectangle([margin + 10 * scale, margin + 10 * scale, sw - margin - 10 * scale, sh - margin - 10 * scale],
                   outline=blend(bg, ink, 0.12), width=1 * scale)

    cx, cy = sw // 2, sh // 2
    variant = (ch_num - 1) % 4

    if is_bridge:
        offset_y = (variant - 1.5) * 10 * scale
        draw.ellipse([cx - 300 * scale, cy - 240 * scale + offset_y, cx + 300 * scale, cy + 240 * scale + offset_y],
                     fill=blend(bg, ink, 0.08))

        lx, ly = cx - 180 * scale, cy + 20 * scale + offset_y
        draw.rounded_rectangle([lx - 120 * scale, ly - 160 * scale, lx + 120 * scale, ly + 180 * scale],
                               radius=60 * scale, fill=blend(bg, ink, 0.16))
        draw.rounded_rectangle([lx - 90 * scale, ly - 130 * scale, lx + 90 * scale, ly + 150 * scale],
                               radius=45 * scale, fill=bg)
        draw.ellipse([lx - 35 * scale, ly - 60 * scale, lx + 35 * scale, ly + 10 * scale], fill=blend(bg, ink, 0.3))

        rx, ry = cx + 180 * scale, cy + 20 * scale + offset_y
        draw.rounded_rectangle([rx - 120 * scale, ry - 160 * scale, rx + 120 * scale, ry + 180 * scale],
                               radius=60 * scale, fill=blend(bg, ink, 0.22))
        draw.rounded_rectangle([rx - 90 * scale, ry - 130 * scale, rx + 90 * scale, ry + 150 * scale],
                               radius=45 * scale, fill=paper)
        draw.ellipse([rx - 35 * scale, ry - 60 * scale, rx + 35 * scale, ry + 10 * scale], fill=blend(bg, ink, 0.35))

        draw.arc([cx - 200 * scale, cy - 100 * scale + offset_y, cx + 200 * scale, cy + 200 * scale + offset_y],
                 start=180, end=0, fill=blend(bg, ink, 0.35), width=3 * scale)
        draw.line([(cx - 360 * scale, cy + 210 * scale + offset_y), (cx + 360 * scale, cy + 210 * scale + offset_y)],
                  fill=blend(bg, ink, 0.25), width=2 * scale)
    else:
        offset_x = (variant - 1.5) * 15 * scale
        draw.ellipse([cx - 340 * scale + offset_x, cy - 280 * scale, cx + 340 * scale + offset_x, cy + 280 * scale],
                     fill=blend(bg, ink, 0.07))
        draw.ellipse([cx - 220 * scale + offset_x, cy - 180 * scale, cx + 220 * scale + offset_x, cy + 180 * scale],
                     fill=blend(bg, ink, 0.14))

        aw = int((140 + variant * 5) * scale)
        draw.rounded_rectangle([cx - aw + offset_x, cy - 160 * scale, cx + aw + offset_x, cy + 180 * scale],
                               radius=aw, fill=blend(bg, ink, 0.22))
        draw.rounded_rectangle([cx - aw + 25 * scale + offset_x, cy - 135 * scale, cx + aw - 25 * scale + offset_x, cy + 155 * scale],
                               radius=aw - 25 * scale, fill=bg)

        draw.ellipse([cx - 40 * scale + offset_x, cy - 60 * scale, cx + 40 * scale + offset_x, cy + 20 * scale],
                     fill=blend(bg, ink, 0.35))
        draw.ellipse([cx - 18 * scale + offset_x, cy - 38 * scale, cx + 18 * scale + offset_x, cy - 2 * scale],
                     fill=paper)

        draw.line([(cx - 300 * scale, cy + 200 * scale), (cx + 300 * scale, cy + 200 * scale)],
                  fill=blend(bg, ink, 0.25), width=2 * scale)
        draw.line([(cx - 240 * scale, cy + 215 * scale), (cx + 240 * scale, cy + 215 * scale)],
                  fill=blend(bg, ink, 0.15), width=1 * scale)
...
class BookRenderer:
    ...
    def prepare_images(self) -> dict[str, Path]:
        manifest_file = BOOK / "assets/images.json"
        if manifest_file.is_file():
            manifest_data = load(manifest_file)
            manifest = manifest_data.get("images", {})
        else:
            manifest_data = {"status": "not_generated", "images": {}}
            manifest = manifest_data["images"]

        required = {"cover", "banner"} | {i["key"] for c in self.chapters for i in c["image_prompts"]}
        assets_img_dir = BOOK / "assets/images"
        assets_img_dir.mkdir(parents=True, exist_ok=True)

        updated_manifest = False
        palettes = self.design["palettes"]
        cover_ink = self.design["ui"]["cover_ink"]

        for key in sorted(required):
            rec = manifest.get(key)
            src = (BOOK / rec["path"]).resolve() if rec and "path" in rec else (assets_img_dir / f"{key}.png").resolve()

            if not src.is_file():
                if key == "cover":
                    synthesize_cover(src, ink_hex=cover_ink, bg_hex="#EAF5FA")
                elif key == "banner":
                    synthesize_banner(src, ink_hex=cover_ink, bg_hex="#EAF5FA")
                else:
                    ch_num = int(key[2:5]) if key.startswith("ch") and key[2:5].isdigit() else 1
                    p = palettes[(ch_num - 1) % len(palettes)]
                    is_bridge = "bridge" in key
                    synthesize_reading(src, is_bridge, ch_num, ink_hex=p["ink"], bg_hex=p["background"])

                manifest[key] = {
                    "key": key,
                    "role": "cover" if key == "cover" else "banner" if key == "banner" else "lesson-hero",
                    "filename": src.name,
                    "path": f"assets/images/{src.name}",
                    "size": "1024x1536" if key == "cover" else "1536x1024",
                    "quality": "low",
                    "model": "pil-editorial-synthesizer",
                }
                updated_manifest = True

        if updated_manifest:
            manifest_data["status"] = "placeholders_synthesized"
            manifest_data["image_count"] = len(manifest)
    ...
    def build(self, only=None):
        ...
        for filename in selected:
            ...
            if filename.endswith('.pdf'):
                ...
                html.write_text('<!doctype html><html lang="bn"><head><meta charset="utf-8"/><title>' + esc(self.copy['title']) + '</title><style>' + self.css(mode) + '</style></head><body>' + body + '</body></html>')
