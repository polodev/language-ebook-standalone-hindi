...
7. Render every filename in book.json expected_output_files: six PDFs + two EPUBs. Build trackers from source chapter titles. Package target fonts and all referenced images in EPUB, check XML and run EPUBCheck. Make banners/screenshots in a separate output subfolder so the publication contract's file count remains unambiguous.
