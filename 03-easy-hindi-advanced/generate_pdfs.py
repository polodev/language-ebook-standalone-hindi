...
def blend_rgb(c1: tuple[int, int, int], c2: tuple[int, int, int], factor: float = 0.5) -> tuple[int, int, int]:
    return tuple(int(a * (1 - factor) + b * factor) for a, b in zip(c1, c2))


def create_editorial_placeholder(key: str, width: int, height: int, palettes: list[dict]) -> Image.Image:
    """Synthesize a clean editorial placeholder image with PIL without text."""
    img = Image.new("RGB", (width, height), (250, 247, 242))
    draw = ImageDraw.Draw(img)
    h = int(hashlib.sha256(key.encode("utf-8")).hexdigest(), 16)
    pal_idx = h % len(palettes)
    pal = palettes[pal_idx]
    pal_next = palettes[(pal_idx + 3) % len(palettes)]
    pal_third = palettes[(pal_idx + 7) % len(palettes)]

    ink1 = hex_to_rgb(pal["ink"])
    bg1 = hex_to_rgb(pal["background"])
    ink2 = hex_to_rgb(pal_next["ink"])
    bg2 = hex_to_rgb(pal_next["background"])
    ink3 = hex_to_rgb(pal_third["ink"])

    # Base warm paper background
    draw.rectangle([0, 0, width, height], fill=(250, 247, 242))

    if key == "cover":
        # 1024 x 1536 portrait. Upper 40% calm and uncluttered for typeset title/subtitle
        draw.rectangle([0, int(height * 0.42), width, height], fill=bg1)
        # Large architectural arch in middle-lower area
        draw.chord([int(width * 0.15), int(height * 0.40), int(width * 0.85), int(height * 0.95)], 180, 360, fill=ink1)
        # Inner contrasting arch
        draw.chord([int(width * 0.28), int(height * 0.52), int(width * 0.72), int(height * 0.95)], 180, 360, fill=bg2)
        # Accent circle / disc
        draw.ellipse([int(width * 0.55), int(height * 0.35), int(width * 0.82), int(height * 0.53)], fill=ink2)
        # Decorative floating pill / card
        draw.rounded_rectangle([int(width * 0.18), int(height * 0.70), int(width * 0.55), int(height * 0.88)], radius=30, fill=ink3)
        draw.rounded_rectangle([int(width * 0.22), int(height * 0.73), int(width * 0.51), int(height * 0.85)], radius=20, fill=(250, 247, 242))
        # Small accent disc
        draw.ellipse([int(width * 0.22), int(height * 0.48), int(width * 0.32), int(height * 0.55)], fill=blend_rgb(ink1, (250, 247, 242), 0.3))
    elif key == "banner":
        # 1536 x 1024 landscape. Left third calm and uncluttered for headline copy
        draw.rectangle([int(width * 0.35), 0, width, height], fill=bg1)
        draw.chord([int(width * 0.40), int(height * 0.15), int(width * 0.95), int(height * 0.95)], 180, 360, fill=ink1)
        draw.rounded_rectangle([int(width * 0.50), int(height * 0.30), int(width * 0.85), int(height * 0.85)], radius=40, fill=bg2)
        draw.ellipse([int(width * 0.65), int(height * 0.18), int(width * 0.90), int(height * 0.55)], fill=ink2)
        draw.rounded_rectangle([int(width * 0.42), int(height * 0.55), int(width * 0.68), int(height * 0.85)], radius=30, fill=ink3)
    else:
        # Chapter reading images (1536 x 1024 landscape): Balanced editorial composition
        draw.rectangle([int(width * 0.08), int(height * 0.08), int(width * 0.92), int(height * 0.92)], fill=bg1)
        layout_style = (h >> 4) % 3
        if layout_style == 0:
            draw.chord([int(width * 0.20), int(height * 0.15), int(width * 0.60), int(height * 0.88)], 180, 360, fill=ink1)
            draw.ellipse([int(width * 0.45), int(height * 0.20), int(width * 0.80), int(height * 0.72)], fill=ink2)
            draw.rounded_rectangle([int(width * 0.30), int(height * 0.45), int(width * 0.70), int(height * 0.85)], radius=30, fill=bg2)
            draw.rounded_rectangle([int(width * 0.35), int(height * 0.50), int(width * 0.65), int(height * 0.80)], radius=20, fill=ink3)
        elif layout_style == 1:
            draw.rounded_rectangle([int(width * 0.15), int(height * 0.18), int(width * 0.45), int(height * 0.82)], radius=40, fill=ink1)
            draw.ellipse([int(width * 0.40), int(height * 0.18), int(width * 0.70), int(height * 0.62)], fill=bg2)
            draw.chord([int(width * 0.55), int(height * 0.25), int(width * 0.85), int(height * 0.85)], 180, 360, fill=ink2)
            draw.ellipse([int(width * 0.22), int(height * 0.30), int(width * 0.38), int(height * 0.54)], fill=(250, 247, 242))
        else:
            draw.ellipse([int(width * 0.18), int(height * 0.22), int(width * 0.52), int(height * 0.72)], fill=ink1)
            draw.rounded_rectangle([int(width * 0.42), int(height * 0.20), int(width * 0.82), int(height * 0.80)], radius=50, fill=bg2)
            draw.chord([int(width * 0.48), int(height * 0.30), int(width * 0.78), int(height * 0.80)], 180, 360, fill=ink2)
            draw.ellipse([int(width * 0.62), int(height * 0.15), int(width * 0.82), int(height * 0.45)], fill=ink3)
        draw.rectangle([int(width * 0.08), int(height * 0.08), int(width * 0.92), int(height * 0.92)], outline=blend_rgb(ink1, (250, 247, 242), 0.7), width=3)
    return img
...
class BookRenderer:
    ...
    def prepare_images(self):
        ...
        manifest_data = load(manifest_file) if manifest_file.exists() else {"status": "not_generated", "images": {}}
        manifest = manifest_data.get("images", {})
        required = {"cover", "banner"} | {i["key"] for c in self.chapters for i in c["image_prompts"]}

        manifest_changed = False
        for key in sorted(required):
            path_str = manifest.get(key, {}).get("path")
            src = (BOOK / path_str).resolve() if path_str else None
            if src is None or not src.is_file():
                w, h = (1024, 1536) if key == "cover" else (1536, 1024)
                img = create_editorial_placeholder(key, w, h, self.design["palettes"])
                target_rel = f"assets/images/{key}.png"
                target_path = BOOK / target_rel
    ...
    def words(self, items):
        return "".join(
            ...
            + '</span> <span class="cue" lang="bn">(' + esc(w["bangla_pronunciation"]) + ", " + esc(w["romanization"]) + ")</span></span>" + esc(w.get("separator_after", " ") or "")
            for w in items
        )
    ...
    def ordered_entry(self, item):
        return (
            self.row("meaning", item["meaning_bengali_md"], True)
    ...
    def cover(self, mode, epub=False):
        return (
            '<section class="cover ' + mode + '">' + self.picture("cover", self.copy["title_bengali"], epub)
            + '<div class="cover-copy"><div class="eyebrow">' + esc(self.copy["edition_note"]) + "</div><h1>"
            + esc(self.copy["title"]) + "</h1><h2>" + esc(self.copy["title_bengali"]) + "</h2><p>"
            + esc(self.copy["subtitle"]) + '</p><p class="publisher">' + esc(self.copy["publisher"]) + "</p></div></section>"
        )
